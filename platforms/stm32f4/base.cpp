...
int main(void)
{
  ...
  TIM2->PSC = 2 * (HAL_RCC_GetPCLK1Freq() / 1'000'000) - 1;
  TIM2->ARR = 0xFFFFFFFF;
  TIM2->EGR = TIM_EGR_UG;
  TIM2->CR1 = TIM_CR1_CEN;

  /* run cpp_binary_size experiment */
  experiment();

  /* get timer count in microseconds */
  uint32_t us_count = TIM2->CNT;
  (void)us_count;
...
{
  HAL_UART_Transmit(&UartHandle, reinterpret_cast<const uint8_t *>(ptr), len, 1000);

...
void add_crows(std::vector<std::unique_ptr<bird>>& birds, size_t num) {
    for (auto i = 0uz; i < num; ++i) {
        birds.emplace_back(static_cast<bird*>(new crow));
    }
}

void add_ducks(std::vector<std::unique_ptr<bird>>& birds, size_t num) {
    for (auto i = 0uz; i < num; ++i) {
        birds.emplace_back(static_cast<bird*>(new duck));
    }
}

void add_owls(std::vector<std::unique_ptr<bird>>& birds, size_t num) {
    for (auto i = 0uz; i < num; ++i) {
